	...
	var PPT = (function () {
		...
		function buildEducationTables(data, cfg, bLbl, eLbl) {
			...
			groupOrder.forEach(function (grp) {
				...
				// FIX 4: use parent label dynamically instead of hardcoded "EDUCATION"
				var sectionTitle = "EDUCATION";
		...
		function load(fy) {
			...
			frappe.call({
				...
				callback: function (r) {
					...
					// Education/sub-item tables — uses current year config for column labels
					var subHtml = buildEducationTables(d, cCfg, curFY + ' Budget', prvFY + ' Actual');
					$('#ppt-sub-tables').html(subHtml);

					// FIX 3: pass selector string '#id' not DOM element to fixStickySubHeader
					$('#ppt-sub-tables .ppt-table-wrap').each(function () {
						var id = $(this).attr('id');
