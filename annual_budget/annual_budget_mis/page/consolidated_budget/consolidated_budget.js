...
// 		function zeroA() {
// 			return { opex_plan:0, opex_act:0, capex_plan:0, capex_act:0, total_plan:0, total_act:0 };
// 		}
// 		function addA(a, b) {
// 			return {
// 				opex_plan  : a.opex_plan  + b.opex_plan,
...
// 			if (covidRows.length) {
// 				covidRows.forEach(function (r) { out.push(r); });
// 				out.push({ display: 'Total', isTotal: true, vals: addA(normalTotal, covidTotal) });
// 			}
// 			return out;
// 		}
...
// 				(entry.actuals || []).forEach(function (sec) {
// 					var nm = normName(sec.name);
// 					if (nm !== 'OPERATING EXPENSES' && nm !== 'OPERATING  EXPENSES') { return; }
// 					(sec.sub_heads || []).forEach(function (sh) {
// 						var n = (sh.name || '').trim();
// 						if (n && !seen[n]) { seen[n] = true; names.push(n); }
// 					});
...
// 			(actuals || []).forEach(function (sec) {
// 				var nm = normName(sec.name);
// 				if (nm !== 'CAPITAL EXPENSES' && nm !== 'CAPITAL  EXPENSES') { return; }
// 				v += parseFloat(field === 'plan'
// 					? (sec.ytd || 0)
// 					: (sec.total_posted_amt_ytd || 0));
// 			});
...
// 				bodyRows += '<tr class="sinr-brkdwn-plan">' +
// 					'<td style="padding-left:18px;color:#333;">- ' + planLabel + '</td>' +
// 					subHeadNames.map(function (n) { return '<td>' + fmtCr(shPlan[n]) + '</td>'; }).join('') +
// 					'<td>' + fmtCr(opexP)       + '</td>' +
// 					'<td>' + fmtCr(capP)         + '</td>' +
// 					'<td>' + fmtCr(opexP + capP) + '</td>' +
// 					'</tr>';
// 				bodyRows += '<tr class="sinr-brkdwn-act">' +
// 					'<td style="padding-left:18px;color:#555;">- ' + actLabel + '</td>' +
// 					subHeadNames.map(function (n) { return '<td>' + fmtCr(shAct[n]) + '</td>'; }).join('') +
// 					'<td>' + fmtCr(opexA)       + '</td>' +
// 					'<td>' + fmtCr(capA)         + '</td>' +
// 					'<td>' + fmtCr(opexA + capA) + '</td>' +
// 					'</tr>';
...
// 			bodyRows += '<tr class="sinr-gt-plan">' +
// 				'<td style="padding-left:18px;">- ' + planLabel + '</td>' +
// 				subHeadNames.map(function (n) { return '<td>' + fmtCr(gtShPlan[n]) + '</td>'; }).join('') +
// 				'<td>' + fmtCr(gtOpexP)           + '</td>' +
// 				'<td>' + fmtCr(gtCapP)             + '</td>' +
// 				'<td>' + fmtCr(gtOpexP + gtCapP)   + '</td>' +
// 				'</tr>';
// 			bodyRows += '<tr class="sinr-gt-act">' +
// 				'<td style="padding-left:18px;">- ' + actLabel + '</td>' +
// 				subHeadNames.map(function (n) { return '<td>' + fmtCr(gtShAct[n]) + '</td>'; }).join('') +
// 				'<td>' + fmtCr(gtOpexA)           + '</td>' +
// 				'<td>' + fmtCr(gtCapA)             + '</td>' +
// 				'<td>' + fmtCr(gtOpexA + gtCapA)   + '</td>' +
// 				'</tr>';
...
// 					Loader.hide();
// 					var apiData = null;
// 					if (r.message && Array.isArray(r.message)) { apiData = r.message; }
// 					else if (r.message && Array.isArray(r.message.message)) { apiData = r.message.message; }
// 					if (!apiData || !apiData.length) {
// 						$tab.html('<div style="padding:40px;text-align:center;color:#aaa;">No data available.</div>');
// 						return;
// 					}
...
// 					Loader.hide();
// 					var records = null;
// 					if (r.message && r.message.status === 'success') { records = r.message.data; }
// 					else if (r.message && Array.isArray(r.message)) { records = r.message; }
// 					if (!records || !records.length) {
// 						$tab.html('<div style="padding:40px;text-align:center;color:#aaa;">No headcount data available.</div>');
// 						return;
// 					}
...
// 				error: function () {
// 					Loader.hide();
// 					$('#tab-headcount').html('<div style="padding:40px;text-align:center;color:red;">Error loading headcount data.</div>');
// 				}
// 			});
// 		}
...
// 	// =============================================================================
// 	// BUDGET & ESTIMATE MODULE — uses get_unit_wise_plan (table_name_filter='Budget & Estimate')
// 	// =============================================================================

// 	var BudgetEstimate = (function () {

// 		var rawData     = [];   // is_this_sub_item===0 entries, sorted by sequence_id
// 		var currentFY   = '';
// 		var openSec     = {};   // { sectionName: bool }
// 		var openSub     = {};   // { "secName::subName": bool }
// 		var expandItems = false;
// 		var bound       = false;
...
// 		// ── Exclude the API's pre-computed GRAND TOTAL section ────────────────────
// 		function isGrandTotalSection(sec) {
// 			return (sec.sequence_id === 9999) ||
// 				   ((sec.name || '').toUpperCase().replace(/\s+/g, ' ').trim() === 'GRAND TOTAL');
// 		}

// 		// ── Value extractors ───────────────────────────────────────────────────────
// 		// Section-level totals: use ytd (plan) / total_posted_amt_ytd (est)
// 		function secVal(entry, secName, field) {
// 			var v = 0;
// 			(entry.actuals || []).forEach(function (sec) {
// 				if (isGrandTotalSection(sec)) { return; }
// 				if (sec.name !== secName) { return; }
// 				v += parseFloat(field === 'plan'
// 					? (sec.ytd || 0)
// 					: (sec.total_posted_amt_ytd || 0));
...
// 		// Sub-head level totals: use ytd (plan) / total_posted_amt_ytd (est)
// 		function subVal(entry, secName, subName, field) {
// 			var v = 0;
// 			(entry.actuals || []).forEach(function (sec) {
// 				if (isGrandTotalSection(sec)) { return; }
...
// 					v += parseFloat(field === 'plan'
// 						? (sub.ytd || 0)
// 						: (sub.total_posted_amt_ytd || 0));
// 				});
// 			});
// 			return v;
// 		}

// 		// Item level: use ytd (plan) / total_posted_amt (est)
// 		// NOTE: items use total_posted_amt, NOT total_posted_amt_ytd
// 		function itemVal(entry, itemName, field) {
// 			var v = 0;
// 			(entry.actuals || []).forEach(function (sec) {
// 				if (isGrandTotalSection(sec)) { return; }
// 				// Direct section items (e.g. CAPITAL EXPENSES items array)
// 				(sec.items || []).forEach(function (i) {
// 					if (i.name === itemName) {
// 						v += parseFloat(field === 'plan'
// 							? (i.ytd || 0)
// 							: (i.total_posted_amt || 0));
// 					}
// 				});
// 				// Sub-head items (e.g. OPERATING EXPENSES > PROGRAM EXPENSES > items)
// 				(sec.sub_heads || []).forEach(function (sub) {
// 					(sub.items || []).forEach(function (i) {
// 						if (i.name === itemName) {
// 							v += parseFloat(field === 'plan'
...
// 		// Grand total: use GRAND TOTAL section (sequence_id 9999) if present, else sum sections
// 		function grandVal(entry, field) {
// 			var gtSec = null;
// 			(entry.actuals || []).forEach(function (sec) {
// 				if (isGrandTotalSection(sec)) { gtSec = sec; }
// 			});
// 			if (gtSec) {
// 				return parseFloat(field === 'plan'
// 					? (gtSec.ytd || 0)
// 					: (gtSec.total_posted_amt_ytd || 0));
// 			}
// 			var v = 0;
// 			(entry.actuals || []).forEach(function (sec) {
// 				if (isGrandTotalSection(sec)) { return; }
...
// 		// ── Cross-entry totals (Grand Total column) ────────────────────────────────
// 		function secTP(sn)         { var v=0; rawData.forEach(function(e){ v+=secVal(e,sn,'plan'); }); return v; }
// 		function secTE(sn)         { var v=0; rawData.forEach(function(e){ v+=secVal(e,sn,'est');  }); return v; }
...
// 		// ── Per-entity cell HTML (Plan | Est) ──────────────────────────────────────
// 		function secCells(sn) {
// 			var h='';
// 			rawData.forEach(function(e) {
// 				h += '<td style="font-weight:700;">' + formatINR(secVal(e,sn,'plan')) + '</td>' +
// 					 '<td style="font-weight:700;">' + formatINR(secVal(e,sn,'est'))  + '</td>';
// 			});
// 			return h;
// 		}
// 		function subCells(sn, subn) {
...
// 			rawData.forEach(function(e) {
// 				h += '<td class="cb-text-accent">' + formatINR(subVal(e,sn,subn,'plan')) + '</td>' +
// 					 '<td class="cb-text-accent">' + formatINR(subVal(e,sn,subn,'est'))  + '</td>';
// 			});
// 			return h;
// 		}
// 		function itemCells(name) {
// 			var h='';
// 			rawData.forEach(function(e) {
// 				h += '<td>' + formatINR(itemVal(e,name,'plan')) + '</td>' +
// 					 '<td>' + formatINR(itemVal(e,name,'est'))  + '</td>';
// 			});
// 			return h;
// 		}
// 		function grandCells() {
...
// 			rawData.forEach(function(e) {
// 				h += '<td>' + formatINR(grandVal(e,'plan')) + '</td>' +
// 					 '<td>' + formatINR(grandVal(e,'est'))  + '</td>';
// 			});
// 			return h;
// 		}
...
// 		// ── Build row structure from first entry's actuals (skip GRAND TOTAL section) ──
// 		function buildStruct() {
// 			if (!rawData.length) { return []; }
// 			return (rawData[0].actuals || [])
// 				.filter(function (sec) { return !isGrandTotalSection(sec); })
// 				.map(function (sec) {
// 					return {
// 						name      : sec.name,
// 						sub_heads : (sec.sub_heads || []).map(function (sub) {
// 							return {
// 								name : sub.name,
// 								items: (sub.items || []).map(function (i) { return { name: i.name }; })
// 							};
// 						}),
// 						items     : (sec.items || []).map(function (i) { return { name: i.name }; })
// 					};
...
// 			$r1.append(
// 				'<th rowspan="2" style="text-align:left !important;min-width:280px;width:280px;' +
// 				'position:sticky;left:0;z-index:50;background:#0076B6;vertical-align:middle;">' +
// 				'Expense Head / Line Item</th>'
// 			);
...
// 			rawData.forEach(function () {
// 				$r2.append(
// 					'<th style="text-align:center;min-width:130px;">' + pl() + '</th>' +
// 					'<th style="text-align:center;min-width:130px;">' + el() + '</th>'
// 				);
// 			});
// 			$r2.append(
// 				'<th style="text-align:center;min-width:130px;background:#004F8B;">' + pl() + '</th>' +
// 				'<th style="text-align:center;min-width:130px;background:#004F8B;">' + el() + '</th>'
// 			);
...
// 					'<td style="text-align:left;"><span class="cb-arrow">' + (secOpen ? '▼' : '▶') + '</span> ' + sn + '</td>' +
// 					secCells(sn) +
// 					tc2(secTP(sn), secTE(sn), 'be-grand-col') +
// 					'</tr>'
// 				);

// 				// Sub-heads (e.g. PROGRAM EXPENSES, PEOPLE EXPENSES under OPERATING EXPENSES)
// 				sec.sub_heads.forEach(function (sub) {
// 					var sk      = sn + '::' + sub.name;
// 					var subOpen = expandItems || (openSub[sk] === true);
// 					var itmVis  = (secOpen && subOpen) ? '' : 'display:none;';
...
// 						'<td style="padding-left:22px;text-align:left;"><span class="cb-arrow">' + (subOpen ? '▼' : '▶') + '</span> ' + sub.name + '</td>' +
// 						subCells(sn, sub.name) +
// 						tc2(subTP(sn, sub.name), subTE(sn, sub.name), 'be-grand-col') +
// 						'</tr>'
// 					);
...
// 							'<td style="padding-left:42px;text-align:left;">' + item.name + '</td>' +
// 							itemCells(item.name) +
// 							tc2(iTotP(item.name), iTotE(item.name), 'be-grand-col') +
// 							'</tr>'
// 						);
// 					});
// 				});

// 				// Direct items under section (e.g. all items under CAPITAL EXPENSES which has no sub_heads)
// 				sec.items.forEach(function (item) {
// 					if (term && item.name.toLowerCase().indexOf(term) === -1) { return; }
// 					$tb.append(
// 						'<tr class="be-item-row be-sec-child be-direct-item" data-sec="' + sn + '" style="' + secVis + '">' +
...
// 			// Grand total row — uses the API's own GRAND TOTAL section values
// 			$tb.append(
// 				'<tr class="cb-row-grand">' +
// 				'<td style="text-align:left;">GRAND TOTAL</td>' +
// 				grandCells() +
// 				tc2(allGP(), allGE(), 'be-grand-col') +
// 				'</tr>'
// 			);
// 		}
...
// 					financial_year    : fy,
// 					month             : 'March',
// 					table_name_filter : 'Budget & Estimate'
// 				},
// 				callback: function (r) {
// 					Loader.hide();
...
// 					if (!apiData || !apiData.length) {
// 						frappe.msgprint('No data returned for Budget & Estimate.');
// 						renderTable();
// 						return;
// 					}

// 					// Only main entries (is_this_sub_item===0), sorted by sequence_id
// 					rawData = apiData
// 						.filter(function (e) { return e.is_this_sub_item === 0; })
// 						.sort(function (a, b) { return (a.sequence_id || 0) - (b.sequence_id || 0); });
...
frappe.pages['consolidated-budget'].on_page_load = function (wrapper) {
	...
	// =============================================================================
	// PPT MODULE — uses get_foundation_overall API
	//
	// API response per entry:
	//   entry.actuals[]          → current FY data  (Budget = ytd, Est = total_posted_amt_ytd)
	//   entry.previous_actuals[] → previous FY data (Budget = ytd, Est = total_posted_amt_ytd)
	//   entry.is_this_sub_item   → 1 = sub-item (indented), 0 = main row
	//   GRAND TOTAL section      → sequence_id 9999 (pre-computed totals)
	//
	// Table 1 (current FY):  Budget = actuals[].ytd,          Est = actuals[].total_posted_amt_ytd
