...
// 		/* Build prevLookup: unit label -> { sub_head_name: {b,a} } */
// 		var prevLookup={};
// 		(prevData||[]).forEach(function(entry){
// 			var lbl=(entry.label||'').trim(); if(!lbl)return;
// 			var pm={};
...
// 		/* Total Foundation card (full width) */
// 		var gridHtml = allCatOrder.length
// 			? buildCard('Total Foundation', allCatOrder, allCurMap, allPrevMap, 'total-foundation', false, true)
// 			: '';
...
// 			var isSub=entry.is_this_sub_item===1;
// 			var pmClean={};
// 			Object.keys(pm).forEach(function(k){pmClean[k]=pm[k];});
// 			gridHtml+=buildCard(unit, catOrder, curMap, pmClean, unit.replace(/[^a-z0-9]/gi,'-').toLowerCase(), isSub, false);
// 		});
...
// 		/* 3d. Enablers — flat rows, no section headers */
// 		'<div class="sec-wrap">' +
// 		'  <div class="sec-heading"><span class="sh-line1" id="enablers-title">Enablers</span><span class="sh-line2" id="enablers-subtitle">Budget vs. Actuals</span></div>' +
// 		'  <p class="sec-note">&#8377;&nbsp;<strong>Cr.</strong></p>' +
// 		'  <div class="tbl-scroll no-maxh">' +
...
// 					financial_year: fy,
// 					month: month,
// 					// table_name_filter: 'Unit Wise Plan,Opex Capex,Enablers',
// 					table_name_filter: 'Education - District Institutes,Education- Azim Premji Schools,Azim Premji University (Bangalore Campus),Azim Premji University (Bhopal Campus),Azim Premji University (Ranchi Campus),Enablers',
// 					is_previous: 0
// 				},
// 				callback: function(r) {
// 					var d = (r.message && typeof r.message === 'object') ? r.message : {};
...
// 	// Enablers table — flat rows (Functions), no section headers
// 	// API key: "Enablers" under table_name_filter="Enablers"
// 	function renderEnablersBreakup(breakupData, fy, month) {
// 		var ytd = monthYearLabel(month, fy);
// 		$('#enablers-subtitle').text('Budget vs. Actuals \u2013 YTD ' + ytd);
// 		$('#enablers-title').text('Enablers');

// 		// Find Enablers data — check multiple possible locations
// 		var raw = (breakupData && breakupData['Enablers'])
// 			? breakupData['Enablers']
// 			: (breakupData && breakupData['Unit Wise Plan'] && breakupData['Unit Wise Plan']['Enablers'])
// 			? breakupData['Unit Wise Plan']['Enablers']
// 			: null;

// 		var subUnits = raw
// 			? (raw.sub_units || (Array.isArray(raw) ? raw : Object.values(raw)))
// 			: [];

// 		// Filter out CONSOLIDATED TOTAL
// 		subUnits = subUnits.filter(function(su){
// 			var lbl = su.label || su.name || '';
// 			return lbl && lbl !== 'CONSOLIDATED TOTAL' && su.settings_doc !== 'CONSOLIDATED';
// 		});
...
// 		// ── 2-row header — "Functions" label column ──
// 		$('#enablers-tbl thead').empty().append(
// 			'<tr class="hdr-blue">' +
// 			'<th rowspan="2" class="col-lbl bl" style="text-align:left;min-width:240px;">Functions</th>' +
// 			'<th colspan="3">Operating Expense</th>' +
...
// 		// Single Total row — light blue, bold (matching image)
// 		html += mkRow('Total', tot, 'sw-unit-total', false);
...
// 		/* 3d. Enablers — flat sub-unit rows, no section grouping */
// 		'<div class="sec-wrap">' +
// 		'  <div class="sec-heading"><span class="sh-line1" id="enablers-title">Enablers</span><span class="sh-line2" id="enablers-subtitle">Budget vs. Actuals</span></div>' +
// 		'  <p class="sec-note">&#8377;&nbsp;<strong>Cr.</strong></p>' +
// 		'  <div class="tbl-scroll no-maxh">' +
...
// 					month: month,
// 					table_name_filter: 'Unit Wise Plan,Opex Capex,Enablers',
// 					label_filter: 'Education - District Institutes,Education- Azim Premji Schools,Azim Premji University (Bangalore Campus),Azim Premji University (Bhopal Campus),Azim Premji University (Ranchi Campus),Enablers',
// 					is_previous: 0
// 				},
// 				callback: function(r) {
// 					var d = (r.message && typeof r.message === 'object') ? r.message : {};
...
// 		/* 3e. Livelihoods — state-wise breakup */
// 		'<div class="sec-wrap">' +
// 		'  <div class="sec-heading"><span class="sh-line1" id="livelihoods-title">Livelihoods</span><span class="sh-line2" id="livelihoods-subtitle">Budget vs. Actuals</span></div>' +
// 		'  <p class="sec-note">&#8377;&nbsp;<strong>Cr.</strong></p>' +
// 		'  <div class="tbl-scroll no-maxh">' +
...
// 		/* Build prevLookup with items[] per sub_head */
// 		(prevData||[]).forEach(function(entry){
// 			var lbl=(entry.label||'').trim(); if(!lbl)return;
// 			var pm={}, opexTot={b:0,a:0};
// 			(entry.actuals||[]).forEach(function(sec){
...
// 						(sh.items||[]).forEach(function(it){
// 							var iname=(it.name||'').trim(); if(!iname)return;
// 							var existing=pm[n].items.find(function(x){return x.name===iname;});
// 							var ib=parseFloat(it.ytd||0)/10000000;
// 							var ia=parseFloat(it.total_posted_amt||0)/10000000;
// 							if(existing){existing.b+=ib;existing.a+=ia;}
// 							else pm[n].items.push({name:iname,b:ib,a:ia});
// 						});
// 					});
// 				}
// 			});
...
// 						(sh.items||[]).forEach(function(it){
// 							var iname=(it.name||'').trim(); if(!iname)return;
// 							var existing=allCurMap[n].items.find(function(x){return x.name===iname;});
// 							var ib=parseFloat(it.ytd||0)/10000000;
// 							var ia=parseFloat(it.total_posted_amt||0)/10000000;
// 							if(existing){existing.b+=ib;existing.a+=ia;}
// 							else allCurMap[n].items.push({name:iname,b:ib,a:ia});
// 						});
// 					});
// 				}
// 			});
...
// 				/* Fix 1: hide rows where both cur and prev are all zero */
// 				if(!c.b&&!c.a&&!p.b&&!p.a) return;
...
// 				rows+=
// 					'<tr>'+
// 					'<td class="col-lbl" style="text-align:left;font-weight:600;">'+cat+'</td>'+
// 					mkTdCr(c.b,'',cat,'Cur Bud')+
// 					mkTdCr(c.a,'ex-act',cat,'Cur Act')+
// 					'<td class="ex-pct">'+fmtPct(c.a,c.b)+'</td>'+
// 					mkTdCr(p.b,'sep-yr',cat,'Prev Bud')+
...
// 				/* Fix 2: show items[] as indented child rows under each sub_head */
// 				var cItems=c.items||[], pItems=p.items||[];
// 				var allItemNames=[];
// 				var itemSeen={};
// 				cItems.forEach(function(it){var n=(it.name||'').trim();if(n&&!itemSeen[n]){itemSeen[n]=true;allItemNames.push(n);}});
// 				pItems.forEach(function(it){var n=(it.name||'').trim();if(n&&!itemSeen[n]){itemSeen[n]=true;allItemNames.push(n);}});

// 				allItemNames.forEach(function(iname){
// 					var ci=cItems.find(function(x){return x.name===iname;})||{b:0,a:0};
// 					var pi=pItems.find(function(x){return x.name===iname;})||{b:0,a:0};
// 					/* Hide zero item rows */
// 					if(!ci.b&&!ci.a&&!pi.b&&!pi.a) return;
// 					rows+=
// 						'<tr class="r-sub-item">'+
// 						'<td class="col-lbl" style="padding-left:24px;color:#555;">'+iname+'</td>'+
// 						mkTdCr(ci.b,'',iname,'Cur Bud')+
// 						mkTdCr(ci.a,'ex-act',iname,'Cur Act')+
// 						'<td class="ex-pct">'+fmtPct(ci.a,ci.b)+'</td>'+
// 						mkTdCr(pi.b,'sep-yr',iname,'Prev Bud')+
// 						mkTdCr(pi.a,'ex-act',iname,'Prev Act')+
// 						'<td class="ex-pct">'+fmtPct(pi.a,pi.b)+'</td>'+
// 						'</tr>';
// 				});
// 			});
// 			rows+=
...
// 						(sh.items||[]).forEach(function(it){
// 							var iname=(it.name||'').trim(); if(!iname)return;
// 							items.push({name:iname,b:parseFloat(it.ytd||0)/10000000,a:parseFloat(it.total_posted_amt||0)/10000000});
// 						});
// 						curMap[n]={b:b,a:a,items:items};
// 						if(!catSeen[n]){catSeen[n]=true;catOrder.push(n);}
// 					});
// 				}
// 			});
